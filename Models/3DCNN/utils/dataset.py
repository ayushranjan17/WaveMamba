...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    elif (dataset_name == 'hsn'):
        image = io.loadmat(os.path.join(dataset_dir, dataset_name, "2013_IEEE_GRSS_DF_Contest_CASI_349_1905_144.mat"))
        image = image['ans']
        gt = io.loadmat(os.path.join(dataset_dir, dataset_name, "GRSS2013.mat"))
        gt = gt['name']
        labels = ['Undefined','Healthy grass','Stressed grass','Synthetic grass','Trees'
            ,'Soil'
            ,'Water'
            ,'Residential'
            ,'Commercial'
